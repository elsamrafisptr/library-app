...
export class WinstonLogger implements LoggerService {
  ...
  constructor() {
    ...
    // Enable console transport in non-production environments
    if (process.env.NODE_ENV !== 'production') {
      this.logger.add(
        new winston.transports.Console({
          level: 'debug', // Console logs include everything up to 'debug'
        }),
      );
    }
  }

  // The missing 'log' method that NestJS expects
  log(message: string) {
